...
HTTP Request
  ...
  Router          (src/routes/)       — mounts middleware chain + handler
     │
     ▼
Middleware        (src/middlewares/)  — auth · RBAC · validation · multer
     │
     ▼
...
- List all projects the current user is a member of, with live member counts
...
- Filter, retrieve, update, and delete tasks
...
- All members (including Project Admin and Member) can read notes
...
- Global error handler with environment-aware detail (stack only in development)
...
| `SERVER_URL` | No | Public URL of this server (used in file URLs) |
...
|---|---|---|---|
| `GET` | `/` | Any member | List projects the user belongs to |
...
| Method | Endpoint | Role Required | Description |
|---|---|---|---|
| `GET` | `/:projectId` | Any member | List all tasks in project |
...
| Method | Endpoint | Role Required | Description |
|---|---|---|---|
| `GET` | `/:projectId` | Any member | List all notes for project |
...
// 201 Response
{
  ...
  "data": {
    "user": {
      ...
      "avatar": { "url": "https://placehold.co/200x200", "localPath": "" },
